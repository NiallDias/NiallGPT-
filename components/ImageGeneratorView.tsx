...
export const ImageGeneratorView: React.FC = () => {
  ...
  return (
    <div 
        className="flex flex-col items-center w-full h-full transition-colors duration-300"
    >
      <div className="w-full max-w-2xl mb-6 mt-4">
        <h2 
            className={`text-2xl sm:text-3xl font-bold text-center mb-1 animate-bounce-in ${isRgbTheme ? 'animate-rgb-text-shadow' : ''}`}
            style={{ color: isRgbTheme ? undefined : 'var(--image-gen-title)', animationDelay: '100ms' }}
        >
            NiallGPT Image Generator
        </h2>
        <p className="text-center text-sm mb-6 animate-bounce-in" style={{color: 'var(--text-secondary)', animationDelay: '200ms'}}>
            Fine-tune your creations with pro-level controls.
        </p>
        ...
        <div className="space-y-4 p-4 rounded-lg shadow-md animate-bounce-in" style={{backgroundColor: 'var(--bg-secondary)', animationDelay: '300ms'}}>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                ...
                <button
                    ...
                    style={{
                        backgroundColor: isGenerateDisabled ? 'var(--button-accent-disabled-bg)' : 'var(--image-gen-button-bg)',
                        color: isGenerateDisabled ? 'var(--text-secondary)' : 'var(--button-accent-text)',
                        cursor: isGenerateDisabled ? 'not-allowed' : 'pointer',
                    }}
                    onMouseEnter={(e) => { if (!isGenerateDisabled) e.currentTarget.style.backgroundColor = 'var(--image-gen-button-hover-bg)'; }}
                    onMouseLeave={(e) => { if (!isGenerateDisabled) e.currentTarget.style.backgroundColor = 'var(--image-gen-button-bg)'; }}
      ...
      {error && (
        <div className="mt-6 p-4 text-center rounded-lg shadow-md w-full max-w-2xl text-sm transition-colors duration-300 animate-shake"
      ...
      <div className="mt-2 flex-grow w-full max-w-5xl flex flex-col justify-center items-center">
        {isLoading ? (
          <div className="flex flex-col items-center justify-center text-center p-8 w-full max-w-md aspect-square animate-bounce-in" style={{color: 'var(--text-secondary)'}}>
          ...
          <div className="w-full flex flex-col items-center animate-bounce-in">
            ...
            <button
              ...
              style={{ backgroundColor: 'var(--download-button-bg)', color: 'var(--button-accent-text)' }}
              onMouseEnter={(e) => e.currentTarget.style.backgroundColor = 'var(--download-button-hover-bg)'}
              onMouseLeave={(e) => e.currentTarget.style.backgroundColor = 'var(--download-button-bg)'}
        ...
        <div className="w-full max-w-7xl mt-12">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                {imageGallery.map((img, index) => {
                    ...
                    return (
                    <div key={img.id} className={`sapphire-card group relative aspect-square overflow-hidden rounded-lg shadow-lg cursor-pointer ${isDeleting ? 'animate-bounce-out' : 'animate-bounce-in'}`} style={{animationDelay: isDeleting ? '0ms' : `${100 + index * 50}ms`, backgroundColor: 'var(--bg-tertiary)'}}>
